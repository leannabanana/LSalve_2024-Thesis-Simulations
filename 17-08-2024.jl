...
pl.plot!(window_sizes, estimate[1][1].* 2 .^(-1.0 .* (window_sizes .- 1)), label=L"μ_2 = μ_1 λ^{-(k - 1)}")
...
pl.plot!(window_sizes, estimate[2][1].* 2 .^(-1.0 .* (exp.(2) .* window_sizes .- 1)), label=L"σ_2 = σ_1 λ^{\exp(2)k - 1}")
...
pl.plot!(window_sizes, gumbel_av[1][1] ./ window_sizes, label = L"μ_2 = \dfrac{μ_1}{k}")
