...
x0 = 1/3
c = 2
start = 0.01 #non-zero initial value
interations = 10^4

#Create T(x) = 3xmod1
y_map = chaotic_map(a, interations)
x_map = range(0, 1, length=length(y))
pl.plot(x_map, y_map)
...
#Observable 2
pl.plot(x_map, y_map, xlimits=(0,1), legend=false)
p2 = scatter!(x_values, y_obs_2, mc =:pink, ms=2, ma=0.5, title=L"\phi = -log(d(x, x_0))")

#Observable 3
pl.plot(x_map, y_map, xlimits=(0,1), label = L"T(x) = 3x \, \, mod \, \, 1", mc=:purple, legend=false)
p3 = scatter!(x_values, y_obs_3, mc =:pink, ms=2, ma=0.5, title=L"\phi = c - d(x, x_0)^{-α}")

### Put 3 plots onto one image 
big_plot = pl.plot(p1, p2, p3, layout=(1,3), size=(1100,400))
savefig(big_plot,"Output_Images/observable_scatter_plots/3_observables.png")
...
d3 = diagnosticplots(fit_obs2)

#Save Diagnostic Tests
draw(PDF("Output_Images/gev_diagnostic_tests/obs1"*Date*".pdf", 25cm, 15cm), d111)
draw(PDF("Output_Images/gev_diagnostic_tests/obs2"*Date*".pdf",25cm, 15cm), d222)
draw(PDF("Output_Images/gev_diagnostic_tests/obs3"*Date*".png", 25cm, 15cm), d333)